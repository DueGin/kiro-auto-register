...
支持三个地区，每个地区都有对应的语言和时区；User-Agent 现在不再从 `config.yaml` 里的固定列表读取，而是按 `browser.user_agent_mode` 动态生成：
...
```
  # auto: 优先使用本机浏览器版本，失败时回退随机 UA
  # local: 强制优先本机浏览器版本，失败时回退随机 UA
  # random: 每次动态生成随机 UA
  user_agent_mode: "auto"
```
